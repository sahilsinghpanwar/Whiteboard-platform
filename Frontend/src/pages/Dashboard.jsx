...
export default function Dashboard() {
  ...
  const handleBoardRenamed = (updated, boardId) => {
    const b = updated?.data ?? updated;
    const targetId = b?._id || boardId;
    if (!targetId) return;
    setBoards((prev) =>
      prev.map((board) => (board._id === targetId ? { ...board, ...b } : board))
    );
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" data-testid="boards-grid">
            {filtered.map((b) => (
              <BoardCard
                ...
                onRenamed={(updated, id) => handleBoardRenamed(updated, id || b._id)}
              />
            ))}
          </div>
        )}
